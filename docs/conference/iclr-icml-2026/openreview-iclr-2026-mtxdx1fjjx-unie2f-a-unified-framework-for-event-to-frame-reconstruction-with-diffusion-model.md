...
score: 5.0
evidence: 利用视频扩散模型与事件帧间残差引导重建帧
tldr: 事件相机仅记录相对强度变化，导致空间信息与静态纹理严重缺失。本文利用预训练视频扩散模型的生成先验，先以事件数据为条件合成视频，再基于事件流与视频帧的物理相关性引入帧间残差引导，重建高保真视频帧。该方法将事件与帧间的依赖关系用于生成建模，为视频帧重建与帧间关系建模提供思路。
source: ICLR-2026-Rejected-Public
selection_source: conference_retrieval
motivation: 事件相机仅记录强度变化，重建视频帧时丢失空间信息与静态纹理。
method: 借助预训练视频扩散模型，并引入基于事件流的帧间残差引导进行条件生成。
result: 方法从稀疏事件数据重建出高保真视频帧，恢复静态纹理与空间细节。
conclusion: 展示了生成先验与帧间物理相关性结合在事件视频重建中的价值。
---
