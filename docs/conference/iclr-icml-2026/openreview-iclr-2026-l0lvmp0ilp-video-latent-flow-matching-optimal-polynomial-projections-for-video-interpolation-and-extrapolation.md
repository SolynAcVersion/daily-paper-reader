...
score: 7.0
evidence: 建模潜变量块的标题引导流并生成概率路径
tldr: 以往视频生成随机采样潜变量块，缺乏对时间连续性的建模。本文提出视频潜空间流匹配VLFM，借助预训练图像生成模型，将潜变量块建模为可解码为时变帧的标题引导流，并用HiPPO框架逼近多项式最优投影以生成概率路径。理论上获得有界通用逼近误差与时间尺度鲁棒性，支撑视频插值与外推，为时序视频建模提供了新范式。
source: ICLR-2026-Rejected-Public
selection_source: conference_retrieval
motivation: 现有视频生成随机采样潜变量块，未显式建模视频随时间的连续演化。
method: 提出视频潜空间流匹配，用HiPPO框架逼近多项式最优投影生成概率路径。
result: 在理论上获得有界通用逼近误差与时间尺度鲁棒性，支持插值与外推。
conclusion: 为基于预训练图像模型的时序视频建模提供了新范式。
---
