...
score: 8.0
evidence: 提升视频生成的物理真实感
tldr: 现代视频生成器在处理复杂物理动态时仍缺乏物理真实感，而现有方法依赖外部验证器或在增强数据上额外训练，代价高昂且难以刻画细粒度运动。本文提出自精炼视频采样，将预训练视频生成器视为去噪自编码器，在推理阶段进行迭代内循环精炼，并引入不确定性感知策略，基于自洽性选择性精炼区域。无需外部验证器或额外训练即可提升细粒度运动与物理真实感，同时避免过度精炼带来的伪影，为视频生成质量提升提供了轻量通用的推理期方案。
source: ICML-2026-Accepted
selection_source: conference_retrieval
motivation: 现代视频生成器难以刻画复杂物理动态，物理真实感不足，而现有方案依赖外部验证器或额外训练，代价高且细粒度有限。
method: 提出自精炼视频采样，将预训练视频生成器当作去噪自编码器，在推理时迭代内循环精炼，并用不确定性感知策略选择性精炼自洽区域。
result: 无需外部验证器或额外训练即可提升细粒度运动与物理真实感，同时抑制过度精炼造成的伪影。
conclusion: 为提升视频生成物理合理性提供了一种轻量、无需额外训练的推理期通用方案。
---
